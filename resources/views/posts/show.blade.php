        ...
        @auth
            @if ($like)
                <form action="{{ route('posts.likes.destroy', [$post, $like]) }}" method="post">
                    ...
                    @method('DELETE')
                    <button type="submit"
                        class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline w-30 mr-2">お気に入り解除</button>
                </form>
            @else
                <form action="{{ route('posts.likes.store', $post) }}" method="post">
                    @csrf
                    <button type="submit"
                        class="bg-indigo-400 hover:bg-indigo-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline w-30 mr-2">お気に入り</button>
                </form>
            @endif
            <span class="text-gray-600">お気に入り数: {{ $post->likes->count() }}</span>
        @endauth
